...
FANTASYFOLIO_KEY_NAME = "fantasyfolio_backup_key"  # Legacy: also checks dam_backup_key


def get_dam_key_path() -> Path:
    """Get path to FantasyFolio's SSH key."""
    return DEFAULT_KEY_DIR / FANTASYFOLIO_KEY_NAME
...
def generate_key(comment: str = "FantasyFolio Backup Key") -> Dict:
    """
    Generate a new SSH key pair for FantasyFolio backups.
...
def delete_key() -> Dict:
    """
    Delete the FantasyFolio SSH key pair.
        ...
        if pub_path.exists():
            try:
                ...
                keys.append({
                    ...
                    'type': key_type,
                    'comment': comment,
                    'is_fantasyfolio_key': path.name == FANTASYFOLIO_KEY_NAME
                })
            except Exception:
                pass
    
    return sorted(keys, key=lambda k: (not k['is_fantasyfolio_key'], k['name']))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--check', action='store_true', help='Check if FantasyFolio key exists')
    parser.add_argument('--generate', action='store_true', help='Generate FantasyFolio key')
    parser.add_argument('--delete', action='store_true', help='Delete FantasyFolio key')
    ...
    if args.check:
        ...
        for k in keys:
            dam = " [FantasyFolio]" if k['is_fantasyfolio_key'] else ""
