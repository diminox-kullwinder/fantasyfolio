...
/* assets_fts(title,author,publisher,filename,text_content) */;
CREATE TABLE IF NOT EXISTS 'assets_fts_data'(id INTEGER PRIMARY KEY, block BLOB);
CREATE TABLE IF NOT EXISTS 'assets_fts_idx'(segid, term, pgno, PRIMARY KEY(segid, term)) WITHOUT ROWID;
...
/* models_fts(filename,title,collection,creator) */;
CREATE TABLE IF NOT EXISTS 'models_fts_data'(id INTEGER PRIMARY KEY, block BLOB);
CREATE TABLE IF NOT EXISTS 'models_fts_idx'(segid, term, pgno, PRIMARY KEY(segid, term)) WITHOUT ROWID;
